...
import { errorMessage, usePutData } from "../components/hooks/fetchData";
...
const popupMassege = computed(() => {
  if (!errorMessage.value) {
    return `Thank you for sending me a message`;
  } else {
...
<template>
  <div class="2xl:container 2xl:mx-auto 2xl:my-auto">
    ...
    >
      <main
        class="phone:flex phone:flex-col lg:grid lg:grid-cols-contact-template min-h-1/2 w-full shadow-lg rounded-lg mb-8"
      >
        <div
          class="bg-slate-600 flex flex-col justify-center items-center p-10"
        >
          <PhChatsCircle
            class="phone:h-36 phone:w-36 sm:h-40 sm:w-40 2xl:h-48 2xl:w-48 fill-green-500"
          ></PhChatsCircle>
          <p
            class="phone:text-sm sm:text-base 2xl:text-2xl text-slate-50 text-center font-semibold"
          >
        ...
        <form class="p-14" @submit.prevent="saveUserInfo">
          <h1
            class="phone:text-4xl sm:text-5xl 2xl:text-6xl text-slate-950 font-extrabold text-center mb-8"
          >
            Contact Me
          </h1>
          <input
          ...
          <base-button
            class="card-project-button w-full phone:text-base sm:text-lg 2xl:text-xl font-semibold"
          >
            Submit
          </base-button>
        </form>
      ...
      <footer
        class="p-4 phone:flex phone:flex-col phone:gap-8 lg:flex-row lg:justify-between items-center"
      >
